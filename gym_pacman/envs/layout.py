...
def randomLayout(layout_params, np_random):
    ...
    if by_proportion:
        ...
        food_positions = np.random.choice(
            np.arange(empty_positions[0].shape[0]), npellets
        )
    ...
    filter_ix = np.where(
        np.sum(np.abs(empty_positions - np.expand_dims([start_y, start_x], 1)), axis=0)
        > 2
    )[0]
    empty_positions = empty_positions[:, filter_ix]

    if (
        empty_positions.shape[1] >= nghosts and nghosts > 0
    ):  # if found a proper place to put ghost
    ...
    while len(stack) > 0:
        ...
        for delta in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if next_pos[0] < 0 or next_pos[1] < 0:
                ...
                or maze[next_pos[0], next_pos[1]] == FOOD
            ):
